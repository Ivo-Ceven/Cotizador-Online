/* ============================================================
   NIVELES DE PRECIO (TIERS)  ·  Legamaster
   ------------------------------------------------------------
   El catalogo de Legamaster trae 3 precios por SKU, en la misma
   fila del Excel (ver el importador en catalog.js). Se eligen
   con DOS selectores:
   ...
   Sin nivel DEAL: a diferencia de Poly, este catalogo no trae una
   segunda lista de promociones, asi que no hay rama `deal` que
   filtrar acá.

   Depende de: brand.js (priceTiers), shared/clientes.js.
   Clonado de poly/js/tiers.js.
   ============================================================ */
...
/* El valor GUARDADO sigue siendo 'MANUAL' aunque en pantalla diga "Custom".
   ...
   columna `Nivel de precio` de `cquotes` y se sincroniza con todo el equipo.

   Las constantes y las cuentas viven en `pricing-core.js`, sin DOM, porque el
   cotizador multimarca cotiza SKUs de Legamaster y tiene que dar el mismo
   precio. */
...
function tierDeLinea(it){ return cevenLegamasterTierEfectivo(it, tierGlobal()); }
...
function precioDeCatalogo(sku, tier){
  return cevenLegamasterPrecioEnLista(products, sku, tier);
}

// Los precios de un SKU, para poder mostrarlos en el selector.
function preciosDeCatalogo(sku){
  var p = cevenLegamasterProducto(products, sku);
  return (p && p.precios) || {};
}
...
function repricearLinea(it){
  return cevenLegamasterRepricear(it, products, tierGlobal());
}
