/* ============================================================
   ...
   el mismo documento:
     ...
     cevenImprimirComprobante(qn, _polyComprobanteOpts()) · el 🧾 de cada
   ...
   que ya aceptó Multi al migrar (src/multi/js/pdf.js): no tiene sentido
   convertir con el TC del momento un documento que se puede volver a
   descargar mañana con otro TC.
...
/* Lo único propio de Poly: la columna "Nota" (disponibilidad que tipea el
   vendedor por línea). Objeto NUEVO en cada llamado — cevenComprobanteDoc()
   le cuelga `.doc` cuando encadena varias páginas y no puede ser compartido. */
function _polyComprobanteOpts(){
  return {
...
function buildPDF(){
  ...
  var filas = _polyPdfFilas(qn);
  if(!filas.length){ showToast('La cotización está vacía.'); return; }

  var doc = cevenComprobanteDoc(qn, filas, null, _polyComprobanteOpts());
  if(!doc) return;
