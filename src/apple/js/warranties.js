...
/* Precios de las garantias de 3 años, DERIVADOS de las tablas de CevenCare
   (js/cevencare-datos.js, que index.html carga antes que este archivo).

   Hasta 09/2026 esto era un objeto literal de ~15 KB escrito a mano. Cuando
   llegaba una actualizacion de precios habia que acordarse de tocar los dos
   lugares, y no se toco: la mini-app de garantias cotizaba un valor y el
   auto-sugerido metia otro, sin que nada avisara. Un MacBook Air M4 GL de 3
   años entraba a la cotizacion en USD 65,14 cuando la lista decia 68,30.

   Derivarlo tambien deja el auto-sugerido con TODOS los planes de 3 años, no
   solo los que alguien hubiera copiado: un equipo nuevo en la lista aparece
   solo. La tabla a mano tenia 36 equipos; esta tiene 71.

   Esos 35 de mas son iPads y iPhones, que la copia a mano no incluia. NO se
   filtran al auto-sugerido: findBestMacWarranty() descarta la descripcion si
   dice ipad/iphone, y ademas arma los candidatos con getFamily(), que devuelve
   null para toda clave que no sea neo/studio/mini/imac/MBP/MBA. El filtro de
   "esto es solo para Macs" vive ahi, no aca.

   Dos equipos (iPad Air 13" M2 e iPhone 15 Pro) vienen repetidos en la lista de
   precios. Como la LUT se indexa por nombre, colapsan en uno — las copias son
   identicas, asi que no se pierde nada. */
function _buildMacWarrantyLUT(deviceList){
  var lut = {};
  (deviceList || []).forEach(function(dev){
    var planes = dev.planes || [];
    var gl3 = null, cc3 = null;
    for(var i=0;i<planes.length;i++){
      /* `años` lleva tilde: es la clave tal como viene de la lista de precios. */
      if(planes[i].canal === 'GL' && planes[i].años === 3) gl3 = planes[i];
      if(planes[i].canal === 'CC' && planes[i].años === 3) cc3 = planes[i];
    }
    /* Solo entran los equipos que tienen las DOS variantes. El modal de
       Cliente Final / Canal ofrece siempre GL y CC; con una sola, una de las
       dos opciones agregaria una garantia vacia. */
    if(gl3 && cc3){
      lut[dev.equipo] = {
        gl_plan: dev.gl_plan || '',
        cc_plan: dev.cc_plan || '',
        gl: { sku: gl3.sku, precio: gl3.precio },
        cc: { sku: cc3.sku, precio: cc3.precio }
      };
    }
  });
  return lut;
...
/* DATA_NUEVO = precio de canal, DATA_CF = precio de cliente final (canal +15 %).
   Si el archivo de datos no cargo, quedan vacios y el auto-sugerido no ofrece
   nada — que es el modo correcto de fallar: mejor no sugerir que sugerir un
   precio inventado. */
var MAC_WARRANTIES_3Y = {
  canal: _buildMacWarrantyLUT(typeof DATA_NUEVO !== 'undefined' ? DATA_NUEVO : null),
  cf:    _buildMacWarrantyLUT(typeof DATA_CF    !== 'undefined' ? DATA_CF    : null)
};
