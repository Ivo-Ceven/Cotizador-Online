...
| `pipeline-sku.js` | **Estado propio de una línea de producto** dentro de una fila del pipeline (03/09/2026): el estado del proyecto vale para todos sus artículos menos los configurados en particular. La herencia (`cevenSkuEstado`), los dos colapsos que evitan overrides redundantes (`cevenSkuEstadoSet`), el reindexado de las claves `SKU\|índice` al borrar una línea (`cevenSkuReindex`) y el **reparto del monto de la fila entre los estados de sus líneas** (`cevenSkuRepartoPorEstado`, que suma exactamente `row.monto`), del que salen los KPI del dashboard. El dato vive en `pipeline."skuStatus"`, columna jsonb que existía desde 07/2026 y usaba solo Apple. Carga **después** de `pipeline-status.js`. Cubierto por `scripts/check-pipeline-sku.js` |
